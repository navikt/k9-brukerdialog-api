...
import no.nav.k9brukerdialogapi.ytelse.Ytelse.OPPLAERINGSPENGER
import no.nav.k9brukerdialogapi.ytelse.opplaeringspenger.domene.Søknad
import org.json.JSONObject
import org.slf4j.Logger
import org.slf4j.LoggerFactory

class OpplaeringspengerService(
    private val søkerService: SøkerService,
    private val vedleggService: VedleggService,
    private val kafkaProdusent: KafkaProducer
) {
    private val logger: Logger = LoggerFactory.getLogger(OpplaeringspengerService::class.java)
    private val YTELSE = OPPLAERINGSPENGER
    ...
    suspend fun registrer(
        ...
        validerK9FormatForOLP(k9Format)
    ...
    private suspend fun registrerSøknadMedVedlegg(
        ...
        try {
            kafkaProdusent.produserKafkaMelding(metadata, JSONObject(komplettSøknad.somJson()), OPPLAERINGSPENGER)
        } catch (exception: Exception){
            logger.error("Feilet ved å legge melding på Kafka.")
